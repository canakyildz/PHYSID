...
protocol BreakfastViewDelegate: class {
    func presentFoodThroughBreakfastView(_ nav: UINavigationController)
}

private let reuseIdentifier = "RecipesCell"

class BreakfastView: UIView {
    
    // MARK: - Properties
    var tableView: UITableView!
    var foods = [Food]()
    var user: User?
    weak var delegate: BreakfastViewDelegate?
    private let headerView = BreakfastHeaderView()
...
extension BreakfastView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        delegate?.presentFoodThroughBreakfastView(nav)        
    }
