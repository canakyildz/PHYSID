//
//  WorkoutAllContentsController.swift
//  PHYSID
//
...
class WorkoutAllContentsController: UIViewController {
    ...
    private var inSearchMode: Bool {
        return !headerView.searchGuy.text!.isEmpty || headerView.didSelectFilter == true }
    ...
    func configureNav() {
        ...
        configureNavigationBar(withTitle: "All Contents",titleView: nil, backgroundcolor: .white, titleColor: .black, prefersLargeTitles: false)
...
extension WorkoutAllContentsController: UITableViewDelegate, UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        print("filtered Contents' count is  \(filteredContents.count)")
        return inSearchMode ? filteredContents.count : workoutContents.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: reuseIdentifier, for: indexPath) as! WorkoutCell
        let workoutContent = inSearchMode ? filteredContents[indexPath.row] : workoutContents[indexPath.row]
...
extension WorkoutAllContentsController: WorkoutHeaderDelegate {
    func updateSearchResults(searchContent: String) {
        filteredContents = workoutContents.filter({ $0.title.localizedCaseInsensitiveContains(searchContent) ||  $0.workoutContenttype.localizedCaseInsensitiveContains(searchContent) })
    ...
    func headerSorting(sorterPattern: String) {
        filteredContents = workoutContents.filter({ $0.workoutContenttype.localizedCaseInsensitiveContains(sorterPattern)})
        tableView.reloadData()
...
extension WorkoutAllContentsController: WorkoutContentControllerDelegate {
